...
1. Отримати з Bubble перелік активних Data Provider і наборів параметрів,
   для яких потрібна свіжа вибірка.
...
- вибірка з `listings_count` нижче налаштованого мінімуму все одно
  зберігається, але позначається як недостатня і не може бути затверджена;
...
18. **v1.1:** вибірка з недостатньою кількістю оголошень зберігається, але
    не може бути затверджена;
