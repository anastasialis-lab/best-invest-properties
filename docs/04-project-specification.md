...
> Специфікація спирається на затверджений візуальний прототип: він визначає
> інтерфейс і видиму поведінку. Числа в прототипі — тестові дані.
...
| Роль | Основні права |
|---|---|
| Anonymous visitor | landing, search, public unit detail, investment analysis і score |
...
| Admin | верифікація забудовників, рев'ю й публікація проєктів, рішення щодо introduction, score-модель, користувачі та автоматизації |

Один User може мати кілька ролей, але MVP UI показує один active workspace at a time.

**Адміністратор у MVP — один тип `admin`.** Правило «двох пар очей» не
застосовується; натомість обов'язкові підтвердження для кожної важливої дії та
повна історія в audit log. Admin-простір має навігацію Dashboard · Approvals ·
Investors · Investment Scores.

**Вимога до архітектури:** модель доступів побудована так, щоб пізніше можна
було додати ролі `support`, `reviewer` і `senior_admin` без переробки. Тому
права перевіряються через `admin_permission_keys` у backend workflows, а не
через перевірку «чи роль = admin».
...
| ADR-07a | Вхідні фінансові оцінки формує Make зі збору порівняльних оголошень, а не AI; вони не публікуються без затвердження адміністратором; усі похідні значення й score рахує детермінований сервіс Bubble | принцип «OpenAI не рахує гроші» діє без винятків |
...
мінімальний екран, який додається до прототипу (склад — у §6.4).
...
| P11 | Saved Searches | 09 (панель) | панель на дашборді P10, окремого екрана немає |
| P12 | Enquiry Detail | — | мінімальний екран — §6.4 |
| P13 | Account Settings | 25 | затверджено |
| P14 | Privacy, Terms | 17, 18 | макет затверджено; текст готує юрист |
| P14b | Cookie Policy, Investment/AI Disclaimer | — | мінімальні сторінки — §6.4 |
...
| D03 | Verification Status / Re-upload | 26 | затверджено |
| D04 | Developer Portal | 10 | затверджено |
| D05 | Company Profile | 27 | затверджено |
...
| D11 | Lead Detail / outcome | — | мінімальний екран — §6.4 |

### Admin

| ID | Екран | Прототип | MVP status |
|---|---|---|---|
| A01 | Admin Login / role gate | 31 | затверджено |
| A02 | Dashboard | 12 | затверджено |
| A03 | Unified Approvals Queue | 23 | затверджено; чотири секції (див. §6.1) |
| A04 | Project Review | 28 | затверджено |
...
Admin-навігація має рівно чотири пункти: **Dashboard · Approvals ·
...
(чипи «✉ Investor notified», «✉ Developer notified») і кнопкою **Reopen**
(для Hold / Decline) або **Correct status** (для Connected) — див. FR-11.
Стан «Queue clear» — окремий порожній стан усього екрана.

`Hold`, `Decline`, `Query` і `Reject` вимагають обов'язкового текстового
обґрунтування (модальне вікно не дає підтвердити без нього). `Hold` і `Query`
додатково вимагають follow-up date і відповідального — у MVP відповідальний
підставляється автоматично. `Approve` для introduction і для
project-submission обґрунтування не вимагає, але показує попередження про
незворотність розкриття контактів.
...
2. **Proposed financial estimates** — рядки з позначкою походження:
   ...
   Ці значення формує не AI — див. FR-03a.
...
Перелік станів нижче є обов'язковим обсягом реалізації.
...
Також обов'язкові наскрізні системні стани:
...
### 6.4 Мінімальні екрани

П'ять екранів потрібні для MVP **в мінімальному вигляді** — по одному екрану
кожен, у стилі затвердженого дизайну.
...
| **P14b Cookie Policy + disclaimers** | сторінки зі структурою Privacy/Terms; текст готує юрист. Cookie banner — якщо використовуються необов'язкові analytics або marketing cookies |
...
9. Publish requires admin permission and complete current score/analysis.
...
- усі інші поля опублікованого проєкту заблоковані й показуються в панелі
...
- Фільтри (ліва колонка, застосовуються одразу, без кнопки Apply):

  | Фільтр | Контрол | Значення |
  |---|---|---|
...
- **Пагінація: 8 карток на сторінку**, нумерований пейджер із «Previous / Next»
  і підписом «Showing X–Y of N properties». Infinite scroll не використовується.
...
**Гейтинг.** Пошук, картки, детальна сторінка, score,
...
- **Developer identity is never shown.** Ім'я та контакти забудовника в
  каталозі не показуються; підпис — «Introduced by Best Invest», кожне
  introduction спершу затверджує адміністратор (BR-07). Прапорець
...
- Unavailable/sold/withdrawn link shows current status and alternatives, not generic 404.
  Це стан `Withdrawn / 404` з поясненням і переходом до схожих об'єктів.
...
Score складається з **п'яти** категорій — однаково на екранах Investment
Analysis, Compare, Score Editor і Project Review:
...
- Вимоги до моделі:
  ...
  - для категорії «Risk & Investor Protection» більший бал означає **нижчий**
    ризик — напрям шкали підписаний в UI.
...
- Admin confirmation required before re-score all.
- Previous scores remain in history.

### FR-03b Рубрика оцінювання
...
| Cost/tax tables | офіційні джерела Кіпру та Іспанії, перевірені податковим/юридичним консультантом |
...
пояснює готовий результат. Тому панель на A04 називається **Proposed
financial estimates**, без згадки AI.

**Порядок формування оцінки ринкової оренди:**
...
**Пороги прийнятності вибірки.** Налаштовуються по
...
> офіційні дані по Кіпру та Іспанії, перевірені місцевим юридичним або
> податковим консультантом. Зберігаються як налаштування країни (A11), а не
> зашиваються у формули.
...
- Перемикач стратегії (`Long-term` / `Short-term`) і
...
- Валідація (повідомлення показуються під полем, блок результату
  ...
  Occupancy (50–100%) і management fee (0–25%) обмежуються діапазоном
  слайдера, а не повідомленням про помилку.
...
- Investor registration (екран 08) — поля:
...
- Profile criteria with edit link: Budget, Target yield,
...
- Required fields explicitly marked: кожне поле підписане `Required` / `Optional`.
...
- Документи заявки поділені на три рівні:
  ...
  **Матриця документів налаштовувана.** Перелік не зашивається в код, а
  задається записами з параметрами:
  ...
  Матриця задається окремо для Кіпру та Іспанії; building permit і escrow не
  зашиваються як обов'язкові для всіх країн.
- Private uploads with type/size/progress/cancel/error.
- Application received confirmation and email з референсом виду `APP-0142`.
...
- Reorder photos, choose cover, assign floor plan to Unit Type. Плани (PDF/PNG)
  ведуться окремим списком із призначенням на тип або на весь проєкт.
...
- Explicit actions, no ambiguous button: **Save draft**, **Discard draft**, **Submit for review**, кожна
  з модальним підтвердженням.
- Readiness checklist distinguishes “ready to submit” and “ready to publish”.
  Чотири рядки: Project details, Developer profile, Photos (N of 30),
  Units (N of declared). Проєкт **можна** подати з неповним набором юнітів, попереджаючи в модальному вікні: «12 units declared · 3
...
- Duplicate submit protected by idempotency; референс виду `PRJ-0311`.
...
- Фільтр за типом — п'ять вкладок; плюс фільтри за віком запиту, `overdue`
  (follow-up date у минулому) і статусом.
...
розкриті, а листи надіслані, тому скасування неможливе. Правила:

- підтвердження показується **перед** відправленням, а не після;
- після відправлення доступні `Reopen` (для Hold / Decline) або
  `Correct status` — вони змінюють статус уперед, а не відкочують дію;
- жоден підпис не стверджує, що контакти або листи були відкликані.
...
Фільтр за виконавцем додається пізніше, коли адміністраторів стане більше
ніж один.
...
**Статуси: один канонічний, три набори підписів.**
...
- П'ять слайдерів ваг, діапазон кожного 0–100%, крок 1.
...
- Перерахунок має прогрес («N of 83»), стан завершення (з можливістю
  **Revert** до попередньої версії) і стан помилки з референсом
  (`SCORE-9C04`), кнопками «Retry the remainder» і «Roll back».
- Панель **Impact preview** показує, як зміняться бали трьох об'єктів.

**Історія версій моделі.** Окремий великий екран не потрібен. На сторінці Score Model достатньо невеликої таблиці або
висувної панелі з полями: номер версії · дата · автор · ваги · причина зміни ·
кількість перерахованих об'єктів.

`Revert` **створює нову версію** з параметрами попередньої, а не видаляє
історію (BR-04).

**Правила ваг:**

- діапазон повзунка — **0–100%**, жорсткої межі на окремий критерій немає;
...
Це реалізується двома способами, обидва обов'язкові:
...
New score model creates new scores; it never overwrites historical Listing Score. Model publish requires admin confirmation.
...
- Інтерфейс перемикає компонування на **одній точці — 760 px**
...
Analytics provider and consent mode визначаються до реалізації.

## 13. Delivery plan

### Phase 0 — Підготовка даних

Результати:

- cost/tax tables для Кіпру та Іспанії;
- матриця документів по країнах;
- перелік порталів для збору порівняльних оголошень оренди;
- юридичні тексти, retention і consent.

Exit: frozen v1 schema і seed data.
