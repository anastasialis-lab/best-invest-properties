...
|---|---|---:|---|
| `min_comparable_listings` | number | **5** | нижче цього вибірку **не можна затвердити** взагалі |
| `sufficient_comparable_listings` | number | **10** | нижче цього вибірка затверджується, але позначається як «thin sample» |
| `max_comparable_age_days` | number | **90** | після цього вибірка вважається застарілою і потребує оновлення |

Обґрунтування дефолтів: медіана з чотирьох оголошень не є ринковою —
один викид зсуває її надто сильно, тому 5 — абсолютний мінімум. Від 10
оголошень медіана поводиться стабільно, тому смуга 5–9 лишається робочою,
...
- `listings_count` нижче `Country Config.min_comparable_listings`
  (дефолт **5**) **блокує затвердження** — оцінка на чотирьох оголошеннях не
  є ринковою;
- `listings_count` у смузі від мінімуму до
  `sufficient_comparable_listings` (дефолт **10**) затверджується, але
  позначається як **thin sample**: позначка показується адміністратору на
  екрані рев'ю і додається інвестору як Analysis Fact із тегом `estimate`;
- вибірка, старша за `max_comparable_age_days` (дефолт **90**), отримує
  `is_current = no` і не може бути джерелом для нового Financial Input;
  наявні опубліковані значення при цьому не зникають, але потрапляють у
  чергу на оновлення;
...
Поріг 50% — константа інтерфейсу, а не поле бази: його зміна не потребує
міграції.
